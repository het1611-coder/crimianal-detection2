...
def register(entries, required, menu_var):
    ...
    os.makedirs(path, exist_ok=True)  # Ensure parent directories are created
    ...
    if len(no_face) > 0:
        ...
        if rowId > 0:
            ...
            cv2.imwrite(os.path.join(profile_pics_path, "criminal %d.png" % rowId), img_list[profile_img_num])
